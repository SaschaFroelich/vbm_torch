...
class Vbm_2days_nolr(Vbm_1day):
    ...
    def locs_to_pars(self, locs):
        param_dict = {'lr': torch.sigmoid(locs[..., 0]),
                    ...
                    'omega_day2': torch.sigmoid(locs[..., 3]),
                    'dectemp_day2': torch.exp(locs[..., 4])}
...
class Repbias_2days_nolr(Vbm_1day):
    ...
    def pars_to_locs(self, df):
        ...
        locs[:, 3] = torch.log(torch.tensor(param_dict['theta_Q_day2']))
        locs[:, 4] = torch.log(torch.tensor(param_dict['theta_rep_day2']))
...
class Repbias_Seqboost_2days_nlor(Repbias_2days_nolr):
...
class Repbias_Handedness_2days_nolr(Repbias_2days_nolr):
...
class Repbias_Handedness_1day(Repbias_1day):
...
class Repbias_Conflict_2days_nolr(Repbias_2days_nolr):
    ...
    def locs_to_pars(self, locs):
        param_dict = {'lr': torch.sigmoid(locs[..., 0]),
                    'theta_Q_day1': torch.exp(locs[..., 1]),
                    'theta_rep_day1': torch.exp(locs[..., 2]),
                    'theta_conflict_day1': locs[..., 3],
                    
                    'theta_Q_day2': torch.exp(locs[..., 4]),
                    'theta_rep_day2': torch.exp(locs[..., 5]),
                    'theta_conflict_day2': locs[..., 6]}
    
        return param_dict
    
    def compute_probs(self, trial, day, blocktype, **kwargs):
        '''
        ...
        if day == 1:
            conflict_param = self.param_dict['theta_conflict_day1']
            
        else:
            conflict_param = self.param_dict['theta_conflict_day2']
...
class Repbias_Conflict_1day(Repbias_1day):
...
class Repbias_CongConflict_2days_nolr(Repbias_2days_nolr):
    ...
    def compute_probs(self, trial, day, blocktype, **kwargs):
        ...
        inc_bonus = cong_bool*seq_bool*opt1_GD*conflict_value*conflict_param
...
class Repbias_CongConflict_1day(Repbias_1day):
...
class Repbias_OnlyR_2days_nolr(Repbias_2days_nolr):
    '''
        Fit only Random behaviour.
    '''
    ...
    def locs_to_pars(self, locs):
        param_dict = {'lr': torch.sigmoid(locs[..., 0]),
                    'theta_Q_day1': torch.exp(locs[..., 1]),
                    'theta_Q_day2': torch.exp(locs[..., 2])}
    
        return param_dict
    
    def specific_init(self):
        "V(ai) = Θ_r*rep_val(ai) + Θ_Q*Q(ai)"
        self.V = [self.compute_V(self.param_dict['theta_Q_day1']*0, 
                                 self.param_dict['theta_Q_day1'])]
    ...
    def compute_probs(self, trial, day, blocktype, **kwargs):
        ...
        probs = probs*blocktype[None, :, None] + (1-blocktype[None, :, None])*0.5
    ...
    def update(self, choices, outcomes, blocktype, day, **kwargs):
        ...
        **kwargs : TYPE
        ...
        if day == 1:
            lr = self.param_dict['lr']
            theta_Q = self.param_dict['theta_Q_day1']
            theta_rep = self.param_dict['theta_Q_day1']*0

        elif day == 2:
            lr = self.param_dict['lr']*0
            theta_Q = self.param_dict['theta_Q_day2']
            theta_rep = self.param_dict['theta_Q_day2']*0
        ...
        if torch.all(choices == -1) and torch.all(outcomes == -1) and torch.all(blocktype == -1):
            ...
            "Only in case of random block"
            "----- Update GD-values -----"
            # outcome is either -2 (error), 0, or 1
            # assert torch.all(outcomes <= 1)
            # # assert torch.all(outcomes > -1)
            
            lr = lr*blocktype [None,:]
    ...
    def reset(self, locs):
        ...
        "Compute V"
        self.V.append(self.param_dict['theta_Q_day1'][..., None]*0*self.rep[-1] + self.param_dict['theta_Q_day1'][..., None]*self.Q[-1])
...
class OnlyQ_2days_nolr(Vbm_1day):
    param_names = ['lr',
                    'theta_Qcong_day1',
                    'theta_Qrand_day1',
                    'theta_Qinc_day1',
    ...
    def locs_to_pars(self, locs):
        param_dict = {'lr': torch.sigmoid(locs[..., 0]),
                    'theta_Qcong_day1': torch.exp(locs[..., 1]),
                    'theta_Qrand_day1': torch.exp(locs[..., 2]),
                    'theta_Qinc_day1': torch.exp(locs[..., 3]),
                    
                    'theta_Qcong_day2': torch.exp(locs[..., 4]),
                    'theta_Qrand_day2': torch.exp(locs[..., 5]),
                    'theta_Qinc_day2': torch.exp(locs[..., 6])}
    
        return param_dict
    
    def compute_probs(self, trial, day, jokertype, **kwargs):
        '''
        ...
        Vopt1 = theta_Qrand[..., None] * (self.Q[-1][torch.where(mask == 1)]).reshape(self.num_particles, self.num_agents, 1) * (jokertype == 0).type(torch.int)[None,..., None] +\
    ...
    def choose_action(self, trial, day, jokertype, **kwargs):
        '''
        Only execute for num_particles == 1.
    ...
    def update(self, choices, outcomes, blocktype, trialstimulus, jokertype, day, **kwargs):
        '''
        Class Vbm().
...
# class OnlyQ_2days_multlr(Vbm_1day):
#     param_names = ['lrcong',
#                    'lrrand',
#                    'lrinc',
#                     'theta_Qcong_day1',
...
#         "Q and rep"
#         self.Q_init = Q_init
#         self.Qcong = [Q_init] # Goal-Directed Q-Values
#         self.Qrand = [Q_init] # Goal-Directed Q-Values
#         self.Qinc = [Q_init] # Goal-Directed Q-Values
#         self.rep = [torch.ones(self.num_particles, self.num_agents, self.NA)*1/self.NA] # habitual values (repetition values)
...
#     def locs_to_pars(self, locs):
#         param_dict = {'lrcong': torch.sigmoid(locs[..., 0]),
#                       'lrrand': torch.sigmoid(locs[..., 1]),
#                       'lrinc': torch.sigmoid(locs[..., 2]),
#                     'theta_Qcong_day1': torch.exp(locs[..., 3]),
#                     'theta_Qrand_day1': torch.exp(locs[..., 4]),
#                     'theta_Qinc_day1': torch.exp(locs[..., 5]),
                    
#                     'theta_Qcong_day2': torch.exp(locs[..., 6]),
#                     'theta_Qrand_day2': torch.exp(locs[..., 7]),
#                     'theta_Qinc_day2': torch.exp(locs[..., 8])}
...
#         _, mask = self.Qoutcomp(self.Qcong[-1], option1)
#         Vopt1 = theta_Qrand[..., None] * (self.Qrand[-1][torch.where(mask == 1)]).reshape(self.num_particles, self.num_agents, 1) * (jokertype == 0).type(torch.int)[None,..., None] +\
#                 theta_Qcong[..., None] * (self.Qcong[-1][torch.where(mask == 1)]).reshape(self.num_particles, self.num_agents, 1) * (jokertype == 1).type(torch.int)[None,..., None] +\
#                 theta_Qinc[..., None] * (self.Qinc[-1][torch.where(mask == 1)]).reshape(self.num_particles, self.num_agents, 1) * (jokertype == 2).type(torch.int)[None,..., None]
        
#         _, mask = self.Qoutcomp(self.Qcong[-1], option2)
#         Vopt2 = theta_Qrand[..., None] * (self.Qrand[-1][torch.where(mask == 1)]).reshape(self.num_particles, self.num_agents, 1) * (jokertype == 0).type(torch.int)[None,..., None] +\
#                 theta_Qcong[..., None] * (self.Qcong[-1][torch.where(mask == 1)]).reshape(self.num_particles, self.num_agents, 1) * (jokertype == 1).type(torch.int)[None,..., None] +\
#                 theta_Qinc[..., None] * (self.Qinc[-1][torch.where(mask == 1)]).reshape(self.num_particles, self.num_agents, 1) * (jokertype == 2).type(torch.int)[None,..., None]
        
#         probs = self.softmax(torch.stack((Vopt1[:,:,0], Vopt2[:,:,0]), 2))
        
#         if day == 2 and torch.any(jokertype > 0):
#             print(self.Qrand[-1].mean(axis=1))
#             # print(self.Qcong[-1].mean(axis=1))
...
#             "Set repetition values to 0 because of new block"
#             self.rep.append(torch.ones(self.num_particles, self.num_agents, self.NA)/self.NA)
#             self.Qcong.append(self.Qcong[-1])
#             self.Qinc.append(self.Qinc[-1])
#             self.Qrand.append(self.Qrand[-1])
            
#         else:
#             "----- Update Q-values -----"
#             # print(trialstimulus)
#             if torch.any(jokertype == 0):
#                 "Random"
#                 jchoices = choices *(jokertype == 0)
#                 Qout, mask = self.Qoutcomp(self.Qrand[-1], choices)
#                 Qnew = self.Qrand[-1] + self.param_dict['lrrand'][..., None]*(outcomes[None,...,None]-Qout)*mask
#                 if torch.any(Qnew > 1):
#                     raise Exception("Q cannot exceed 1.")
#                 self.Qrand.append(Qnew)
                
#             if torch.any(jokertype == 1):
#                 "Congruent"
#                 jchoices = choices *(jokertype == 1)
#                 Qout, mask = self.Qoutcomp(self.Qcong[-1], choices)
#                 Qnew = self.Qcong[-1] + self.param_dict['lrcong'][..., None]*(outcomes[None,...,None]-Qout)*mask
#                 if torch.any(Qnew > 1):
#                     raise Exception("Q cannot exceed 1.")
#                 self.Qcong.append(Qnew)
                
#             if torch.any(jokertype == 2):
#                 "Incongruent"
#                 jchoices = choices *(jokertype == 2)
#                 Qout, mask = self.Qoutcomp(self.Qinc[-1], choices)
#                 Qnew = self.Qinc[-1] + self.param_dict['lrinc'][..., None]*(outcomes[None,...,None]-Qout)*mask
#                 if torch.any(Qnew > 1):
#                     raise Exception("Q cannot exceed 1.")
#                 self.Qinc.append(Qnew)
...
#             if len(self.Qrand) > 10 & len(self.Qcong) > 10 & len(self.Qinc) > 10:
#                 "Free up memory space"
#                 self.Qrand[0:-2] = []
#                 self.Qcong[0:-2] = []
#                 self.Qinc[0:-2] = []
...
#         "Q and rep"
#         self.Qcong = [self.Q_init.broadcast_to(self.num_particles, self.num_agents, self.NA)] # Goal-Directed Q-Values
#         self.Qinc = [self.Q_init.broadcast_to(self.num_particles, self.num_agents, self.NA)] # Goal-Directed Q-Values
#         self.Qrand = [self.Q_init.broadcast_to(self.num_particles, self.num_agents, self.NA)] # Goal-Directed Q-Values
#         self.rep = [torch.ones(self.num_particles, self.num_agents, 4)*0.25] # habitual values (repetition values)
...
#     def locs_to_pars(self, locs):
#         param_dict = {'beta0': locs[..., 0],
#                       'beta1': locs[..., 1],
#                       'sigma': torch.exp(locs[..., 2])}
        
#         return param_dict
        
#     def compute_like(self, x):
#         mean = self.param_dict['beta0'] + x*self.param_dict['beta1']
#         likelihood = torch.normal(mean, self.param_dict['sigma'])
...
class Repbias_Interaction_2days_nolr(Repbias_2days_nolr):
...
class Repbias_Interaction_1day(Repbias_1day):
    ...
    def compute_probs(self, trial, day, blocktype, **kwargs):
        ...
        interaction_param = self.param_dict['interaction_param']
...
class Bullshitmodel(Repbias_1day):
...
class Repbias_2days_nolr_noQ(Vbm_1day):
    ...
    def locs_to_pars(self, locs):
        param_dict = {'lr': torch.sigmoid(locs[..., 0]),
                    'theta_Q': torch.exp(locs[..., 1]),
                    'theta_rep_day1': torch.exp(locs[..., 2]),
                    'theta_rep_day2': torch.exp(locs[..., 3])}
    ...
    def pars_to_locs(self, df):
        ...
        locs[:, 1] = torch.log(torch.tensor(param_dict['theta_Q']))
        locs[:, 2] = torch.log(torch.tensor(param_dict['theta_rep_day1']))
        locs[:, 3] = torch.log(torch.tensor(param_dict['theta_rep_day2']))
    ...
    def reset(self, locs):
        ...
        "Compute V"
        self.V.append(self.param_dict['theta_rep_day1'][..., None]*self.rep[-1] + self.param_dict['theta_Q'][..., None]*self.Q[-1])
