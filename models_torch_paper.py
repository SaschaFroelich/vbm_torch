...
class OnlyQ_2days_nolr(Vbm):
    ...
    def compute_probs(self, trial, day, jokertype, **kwargs):
        ...
        # Vopt1 = theta_Qrand[..., None] * self.Qrand[-1][:,
        #                                                 torch.arange(self.num_agents),
        #                                                 option1][..., None] * (jokertype == 0).type(torch.int)[None,..., None] +\
            
        #         theta_Qcong[..., None] * self.Qcong[-1][:,
        #                                                 torch.arange(self.num_agents),
        #                                                 option1][..., None] * (jokertype == 0).type(torch.int)[None,..., None] +\
                    
        #         theta_Qrand[..., None] * self.Qrand[-1][:,
        #                                                 torch.arange(self.num_agents),
        #                                                 option1][..., None] * (jokertype == 0).type(torch.int)[None,..., None] +\
        
        # Vopt2 = theta_Qrand[..., None] * self.Qrand[-1][torch.arange(self.num_particles), torch.arange(self.num_agents), option2] * jokertype == 0 +\
        #         theta_Qcong[..., None] * self.Qcong[-1][torch.arange(self.num_particles), torch.arange(self.num_agents), option2] * jokertype == 1 +\
        #         theta_Qinc[..., None] * self.Qinc[-1][torch.arange(self.num_particles), torch.arange(self.num_agents), option2] * jokertype == 2
...
class Repbias_Interaction_2days_nolr(Repbias_1day):
